...
#[contract_state(contract = "CIS1-GG")]
#[derive(Serialize, SchemaType)]
struct State {
...
#[receive(contract = "CIS1-GG", name = "mint", parameter = "MintParams", enable_logger)]
    ...
    } in transfers
    {
        ...
        if let Receiver::Contract(address, function) = to {
            let parameter = OnReceivingCis1Params {
                ...
                amount,
                from,
                contract_name: OwnedContractName::new_unchecked(String::from("init_CIS1-GG")),
                data,
            };
...
#[receive(contract = "CIS1-GG", name = "balanceOf", parameter = "ContractBalanceOfQueryParams")]
...
/// - It fails to parse the parameter.
/// - Message sent back with the result rejects.
#[receive(contract = "CIS1-GG", name = "operatorOf", parameter = "OperatorOfQueryParams")]
...
#[receive(contract = "CIS1-GG", name = "onReceivingCIS1")]
    ...
    fn test_mint() {
        ...
        claim!(
            logger.logs.contains(&to_bytes(&Cis1Event::TokenMetadata(TokenMetadataEvent {
                token_id:     TOKEN_0,
                metadata_url: MetadataUrl {
                    url:  "https://gamers.gold/token/02".to_string(),
                    hash: None,
                },
            }))),
        ...
        claim!(
            logger.logs.contains(&to_bytes(&Cis1Event::TokenMetadata(TokenMetadataEvent {
                token_id:     TOKEN_1,
                metadata_url: MetadataUrl {
                    url:  "https://gamers.gold/token/2A".to_string(),
                    hash: None,
                },
            }))),
