...
const TOKEN_ID_STABLE_COIN: ContractTokenId = TokenIdUnit();

/// The metadata url for the wCCD token.
const TOKEN_METADATA_URL: &str = "https://gamers.gold/token/stable-coin";
...
/// The contract state,
#[contract_state(contract = "CIS1-Stable-Coin")]
#[derive(Serialize, SchemaType)]
struct State {
...
impl State {
    ...
    fn balance(
        ...
        ensure_eq!(token_id, &TOKEN_ID_STABLE_COIN, ContractError::InvalidTokenId);
...
fn contract_wrap<A: HasActions>(
    ...
    // Update the state.
    state.mint(&TOKEN_ID_STABLE_COIN, amount.micro_ccd, &receive_address)?;
...
fn contract_unwrap<A: HasActions>(
    ...
    // Update the state.
    state.burn(&TOKEN_ID_STABLE_COIN, params.amount, &params.owner)?;
...
mod tests {
    ...
    fn initial_state() -> State {
        ...
        state.mint(&TOKEN_ID_STABLE_COIN, 400, &ADDRESS_0).expect_report("Failed to setup state");
        state
    }
    ...
    fn test_init() {
        ...
        let balance0 =
            state.balance(&TOKEN_ID_STABLE_COIN, &ADDRESS_0).expect_report("Token is expected to exist");
    ...
    fn test_transfer_account() {
        ...
        let balance0 =
            state.balance(&TOKEN_ID_STABLE_COIN, &ADDRESS_0).expect_report("Token is expected to exist");
        let balance1 =
            state.balance(&TOKEN_ID_STABLE_COIN, &ADDRESS_1).expect_report("Token is expected to exist");
        claim_eq!(
            balance0,
            300,
